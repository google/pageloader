...
library pageloader.test.sync.basic;
...
void runTests() {
  group('basic tests', () {
    test('simple', () async {
      PageForSimpleTest page = await loader.getInstanceSync(PageForSimpleTest);
      await verifyTable(page.table);
      expect(page.loader, loader);
    });

    test('class annotations', () async {
      Table table = await loader.getInstanceSync(Table);
      await verifyTable(table);
    });

    test('class annotation on nested field', () async {
      PageForClassAnnotationTest page =
          await loader.getInstanceSync(PageForClassAnnotationTest);
      await verifyTable(page.table);
    });

    test('sub-class', () async {
      SubclassPage page = await loader.getInstanceSync(SubclassPage);
      await verifyTable(page.table);
    });

    test('setters', () async {
      PageForSettersTest page =
          await loader.getInstanceSync(PageForSettersTest);
    ...
    test('skip fields without finders', () async {
      PageForSkipFieldsWithoutFinderTest page =
          await loader.getInstanceSync(PageForSkipFieldsWithoutFinderTest);
      expect(page.shouldHaveOneElement, hasLength(1));
      expect(page.shouldBeNull, isNull);
    });

    test('mixin', () async {
      PageForMixinTest page = await loader.getInstanceSync(PageForMixinTest);
      await verifyTable(page.table);
      expect(page.shouldHaveOneElement, hasLength(1));
    ...
    test('private constructor', () async {
      PageForPrivateConstructorTest page =
          await loader.getInstanceSync(PageForPrivateConstructorTest);
    ...
    test('private fields', () async {
      PageForPrivateFieldsTest page =
          await loader.getInstanceSync(PageForPrivateFieldsTest);
      await verifyTable(page.table);
    });

    test('Lazy fields', () async {
      PageForLazyTest page = await loader.getInstanceSync(PageForLazyTest);
    ...
    test('has focus', () async {
      PageForFocusTest page = await loader.getInstanceSync(PageForFocusTest);
    ...
    test('optional with Lazy', () async {
      PageForOptionalFunctionTest page =
          await loader.getInstanceSync(PageForOptionalFunctionTest);
    ...
    test('nbsp in text', () async {
      PageForNbspTest page = await loader.getInstanceSync(PageForNbspTest);
    ...
    test('optional', () async {
      PageForOptionalElementTest page =
          await loader.getInstanceSync(PageForOptionalElementTest);
